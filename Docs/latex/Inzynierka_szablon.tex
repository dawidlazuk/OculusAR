...
Niniejsza praca ma na celu przedstawienie możliwości wykorzystania gogli wirtualnej rzeczywistości do realizacji efektu rzeczywistości rozszerzonej. System stworzony w ramach tej pracy, umożliwia przechwytywanie obrazu z dwóch kamer, a następnie przesyłanie go do ludzkich oczu za pomocą gogli wirtualnej rzeczywistości. Założeniem pracy było uzyskanie efektu widzenia stereoskopowego przy użyciu stworzonych gogli.
Praca porusza kwestie dotyczące problemów, powodowanych przez odmienne cechy obrazu cyfrowego i obrazu rzeczywistego oraz różnice, między ludzkim zmysłem wzroku, a osiąganym przez skonstruowane gogle. 

Integralną częścią systemu, stworzonego na potrzeby pracy, jest warstwa przetwarzania obrazu, umożliwiająca aplikowanie wcześniej zdefiniowanych przekształceń do obrazu, podczas jego przesyłania z kamer do gogli.\\

\noindent \textbf{Słowa kluczowe:} wirtualna rzeczywistość, rozszerzona rzeczywistość, przetwarzanie obrazu, zmysł wzroku, widzenie stereoskopowe
\end{abstract}
...
The purpose of this thesis is to present possible application of the virtual reality goggles into an implementation of augmented reality effect. The system created, as the main part of this thesis, enables user to capture images from two cameras and then apply them to human eyes with the use of virtual reality goggles. This thesis also addresses issues related to problems caused by different features of digital image and real image and differences between human vision and achieved result with the constructed goggles.

The integral part of the system, described by this thesis, is the image processing layer, enabling user to apply defined transformations into the acquired images, during its transmission from cameras to goggles. \\

\noindent \textbf{Keywords:} virtual reality, augmented reality, image processing, human vision, stereoscopic vision \end{abstract}}
...
Za implementację i opis modułu przechwytywania obrazu z kamer oraz serwisu komunikacji międzyprocesowej pomiędzy dwoma aplikacjami, na które się składa system, był odpowiedzialny Dawid Łazuk. Wykonał i opisał testy wydajnościowe systemu oraz napisał rozdział \textit{Technologia VR/AR, Zmysł wzroku}.

Za implementację i opis modułu przekazywania obrazu do gogli oraz aplikacji konfiguracyjnej był odpowiedzialny Piotr Piwowarski. Przeprowadził i opisał testy użytkowe skonstruowanych gogli, a także opisał wnioski z pracy i wizję na przyszłość stworzonego rozwiązania.
...
Badania antropometryczne personelu armii Stanów Zjednoczonych w 2012 roku\cite{IPdistance} objęły swoim zakresem odległości między ludzkimi źrenicami. Wyniki tych badań są zawarte w~tabeli~\ref{IPdistanceTable}. Dane są przedstawione w milimetrach.
...
\end{tabular}
\caption{Odległości między źrenicami na podstawie \cite{IPdistance}}
\label{IPdistanceTable}
\end{table}

Z danych przedstawionych w tabeli \ref{IPdistanceTable} jednoznacznie wynika, że odległość pomiędzy ludzkimi źrenicami zawiera się w przedziale od 51.0 milimetrów do 77.0 milimetrów.
...
\caption[Zbieżność oczu\cite{zbieznosc}]{Zbieżność oczu \cite{zbieznosc}}
\label{zbieznoscRysunek}
\
\end{figure}

Na rysunku \ref{zbieznoscRysunek} punkt $O$ odpowiada punktowi dali wzrokowej, a punkt $O'$ punktowi bliży wzrokowej. 
...
System składa się z modułów realizujących ściśle określone zadania, dlatego też łatwo wydzielić je do osobnych projektów. Schemat na rysunku \ref{schematArchitektury} przedstawia architekturę rozwiązania wraz z zaznaczonym kierunkiem przekazywania obrazu pomiędzy warstwami. 
...
Jak widać na rysunku \ref{schematArchitektury}, obraz zanim trafi do oka przechodzi przez wszystkie warstwy. Najpierw przy użyciu biblioteki EmguCV obraz jest pobierany z kamer. Następnie przy użyciu stworzonych na potrzeby systemu klas, stosowane są dla niego odpowiednie przekształcenia. Końcowy efekt osiągany jest poprzez przesłanie przetworzonego obrazu do gogli VR, za~pomocą aplikacji działającej przy użyciu silnika Unity. Aplikacja konfiguracyjna, która została stworzona z wykorzystaniem technologii WPF, umożliwia modyfikację parametrów w warstwach przechwytywania i przetwarzania obrazu, podczas działania systemu. 
...
W tym module zaimplementowane są wszystkie operacje powiązane z kamerami. Moduł jest odpowiedzialny za:
\begin{itemize}
...
Kontrakt tego modułu przedstawiony jest na rysunku \ref{kontraktViewProvider}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.9]{images/IViewProvider}
\caption[Interfejsy przechwytywania obrazów]{Interfejsy przechwytywania obrazów}
\label{kontraktViewProvider}
\end{figure}
...
Poniżej są przedstawione zasady działania operacji zaimplementowanych w tym module:
\begin{description}
...
\caption[Interfejsy przetwarzania obrazów]{Interfejsy przetwarzania obrazów}
\end{figure}
...
\caption[Interfejs obiektu przekształcającego obraz]{Interfejs obiektu przekształcającego obraz}
\end{figure}
...
Najważniejsze operacje modułu to:
\begin{itemize}
\item Przekazanie i wyświetlanie obrazu
\item Konwersja obrazu
\end{itemize}

Unity3D jest silnikiem grafiki 3D i w podstawowej wersji nie wspiera obsługi urządzeń VR. Jednak producenci gogli zapewniają dedykowane biblioteki programistyczne dla swoich urządzeń, oparte na tym silniku. Pozwalają one na dodanie obsługi wirtualnej rzeczywistosci oraz sterowanie przekazywanym obrazem, dla każdego oka osobno. Wykorzystany został pakiet \textit{Oculus Utitites for Unity} udostępniany przez producenta gogli Oculus Rift\footnote{https://developer.oculus.com/downloads/unity/}, który w  edytorze silnika Unity funkcjonuje pod nazwą \textit{OVR plugin}.  \hfill \\

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{images/unityscene}
\caption[Scena w edytorze Unity]{Scena w programie Unity Editor}
\end{figure}
...
W module zostały zdefiniowane następujące interfejsy, zapewniające metody do realizacji kluczowych jego elementów.

\begin{figure}[h]
\centering
\includegraphics[scale=0.9]{images/IStereoVidTransmitter}
\caption[Interfejs zwracający tekstury do wyświetlenia]{Interfejs zwracający tekstury do wyświetlenia}
\end{figure}
...
\caption[Interfejs konwersji tekstur]{Interfejs dokonujący konwersji obrazu do tekstury}
\end{figure}
...
\caption[Interfejs aplikacji konfiguracyjnej]{Interfejs aplikacji konfiguracyjnej}
\end{figure}
...
\caption[Widok kontrolek kanałów]{Kontrolki \textit{ChannelControl}.}
\end{figure}
...
\caption[Widok kontrolki ustawień]{Kontrolka \textit{SettingsControl}.}
\end{figure}
...
\caption[Interfejsy serwisów]{Interfejsy serwisów}
\end{figure}
...
\caption[Projekt uchwytu]{Projekt uchwytu (autor: Maciej Spychała)}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.25]{images/delayGray}
\caption[Test opóźnień]{Test opóźnień (obraz w skali szarości)}
\end{figure}
...
\newpage
W tabelach \ref{opoznienia1440x1080} i \ref{opoznienia640x480} przedstawione są wyniki pomiarów dla obu rozdzielczości. Na postawie tych pomiarów wyznaczono średnie opóźnienia dla każdego przypadku. Średnie opóźnienia podane są w tabeli \ref{srednieOpoznienia}.
...
\label{srednieOpoznienia}
\end{table}


\item[Liczba klatek na sekundę] \hfill \\
Następną kwestią wydajnościową jest liczba klatek na sekundę (FPS). Odpowiednio wysoka liczba FPS jest niezbędna, aby osiągnąć efekt płynności obrazu, na który użytkownik patrzy. Jeżeli liczba ta będzie niewystarczająca osoba, która korzysta z~gogli, będzie miała wrażenie zacinania się obrazu. To zjawisko jest zwane potocznie ,,klatkowaniem'', od przyczyny, która je powoduje.

\textit{Sposób wykonania testu}

Do głównej pętli aplikacji, przetwarzającej klatki, dodano niewielką ilość kodu, który zlicza liczbę iteracji pętli (odświeżeń obrazu) i co ustaloną liczbę wypisuje aktualny czas na~konsolę. Operacje te nie pobierają dużo mocy obliczeniowej na ich wykonanie zatem można założyć, że ich wpływ na wynik pomiarów jest pomijalny. Dzieląc ustaloną liczbę klatek przez różnicę czasów (w sekundach) wypisanych w dwóch kolejnych iteracjach otrzymano liczbę klatek na sekundę.
...
W tabelach \ref{fps1440x1080} i \ref{fps640x480} przedstawione są wyniki pomiarów dla obu rozdzielczości. Na postawie tych pomiarów wyznaczono średnie liczby FPS dla każdego przypadku. Średnie liczby FPS podane są w tabeli \ref{srednieFPS}.
...
\end{tabular}
\caption{Pomiary liczby FPS (rozdzielczość 1440 x 1080)}
\label{fps1440x1080}
\end{table}
...
\end{tabular}
\caption{Pomiary liczby FPS (rozdzielczość 640 x 480)}
\label{fps640x480}
\end{table}
...
\end{tabular}
\caption{Średnie liczby FPS}
\label{srednieFPS}
\end{table}

Z liczby FPS można wyznaczyć liczbę milisekund, która jest potrzebna do przetworzenia jednej klatki. Czas ten jest podany w tabeli \ref{czasNaKlatke}.
...
\end{tabular}
\caption{Średnia liczba milisekund na przetworzenie jednej klatki}
\label{czasNaKlatke}
\end{table}
...
\caption[Gra w łapki]{Gra w łapki}
\end{figure}
...
\caption[Chwytanie przedmiotu]{Chwytanie przedmiotu}
\end{figure}

\item[Celowanie laserem w punkt] \hfill \\
\textit{Opis} \\
...
\caption[Rzucanie piłką do celu]{Rzucanie piłką do celu}
\end{figure}
...
\subsection{Pozytywne rezultaty}

Większość istniejących projektów dotyczących wirtualnej rzeczywistości wykorzystuje tylko jedną kamerę, dzięki czemu łatwiej jest uzyskać dobrą wydajność i sterować obrazem, ale obraz ten nie zachowuje głębi właściwej dla rzeczywistego postrzegania przez człowieka.
Celem tej pracy było zbadanie możliwości rozszerzonej rzeczywistości w oparciu o widzenie stereoskopowe i udało się to zrealizować.

\begin{description}

\item[Uzyskanie efektu głębi obrazu] \hfill \\
Dzięki wykorzystaniu dwóch kamer, a także odpowiedniemu ich ustawieniu, obraz trafiający do oczu daje pożądany efekt głębi obrazu. Było to jednym z głównych celów niniejszej pracy. 

\item[Rozszerzalna warstwa przetwarzania obrazu] \hfill \\
Mając na uwadze dalsze rozwijanie stworzonego systemu, stworzona została warstwa przetwarzania obrazów, która może być rozbudowywana o nowe funkcjonalności. Przygotowane ramy pod tworzenie nowych przekształceń nie narzucają poważnych ograniczeń i mogą posłużyć do stworzenia dowolnych efektów. 

\item[Sterowanie parametrami systemu] \hfill \\
W pierwotnych założeniach projektu nie było wymagania, aby konfiguracja systemu była zrealizowana w postaci osobnego procesu, jednak rozwiązanie to wnosi istotne korzyści. Proces sterowania parametrami obrazu może odbywać się na innym komputerze, niż jego pobieranie i przekazywanie do gogli. Aplikację konfiguracyjną można również wyłączyć, dzięki czemu nie będzie zużywała zasobów komputera, wpływając tym samym na wydajność systemu. 

\item[Montaż kamer na goglach] \hfill \\
Udało się zamontować kamery na goglach tak, aby ich położenie odpowiadało położeniu oczu człowieka. Uchwyt do kamer, stworzony na potrzeby pracy, zostanie w~laboratorium Koła Naukowego Wirtualnej Rzeczywistości, dzięki czemu będzie mógł zostać wykorzystany przy innych tego typu projektach.

\end{description}

\subsection{Problemy}

W systemie pojawiły się różnego rodzaju problemy i ograniczenia. Dotyczyły one kwestii technicznych wykorzystanego sprzętu oraz wyboru zbyt złożonych technologii do realizacji niektórych funkcjonalności.

\begin{description}

\item[Wydajność systemu] \hfill \\
W stworzonym rozwiązaniu nie udało się zapewnić wysokiej wydajności systemu. Wykonywanie prostych manualnych czynności jest utrudnione, ze względu na opóźnienie obrazu. Precyzyjne ruchy ciała w oparciu o bodźce wzrokowe, możliwe są jedynie przy wykonywaniu ich bardzo wolno. U użytkownika występuje odruchowe wspomaganie kontroli ruchów innymi zmysłami niż wzrokowy, w stopniu znacznie większym niż ma to miejsce w~rzeczywistości.

\item[Odwzorowanie ruchu gałek ocznych] \hfill \\
Ze względu na techniczne ograniczenia wykorzystywanych gogli, system nie posiada możliwości śledzenia ruchu gałek ocznych. Powoduje to rozbieżności w odczuciach dotyczących obrazu, porównując do rzeczywistego widzenia. Ludzkie oczy poruszają się, dostosowując się do odległości obiektu na który patrzą. W stworzonym rozwiązaniu odtworzenie tej cechy polegałoby na odpowiednim poruszaniu kamer umieszczonych na~goglach, jednak poza zakresem niniejszej pracy. Pożądany efekt był osiągany poprzez ręczną korekcję kamer w razie potrzeby.

\item[Stabilność systemu] \hfill \\
Działanie systemu bywa niestabilne, związane jest to z kilkoma czynnikami. Pośrednio wykorzystywane są sterowniki systemowe dotyczące urządzeń przechwytywania obrazu. Te~mogą być różne w zależności od wykorzystywanych kamer lub komputera. Z tego powodu warstwa przechwytywania obrazu może powodować błędy na komputerach innych niż ten, na którym prowadzony był główny proces tworzenia pracy oraz testów z goglami wirtualnej rzeczywistości. Za nieprzewidziane błędy w aplikacji odpowiadają również wykorzystywane biblioteki, będące wciąż w fazie rozwoju i posiadają wewnętrzne błędy, które powodują, czasami niemożliwe do~usunięcia, błędy w tworzonym systemie.
