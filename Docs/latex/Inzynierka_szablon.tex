...
Samo Unity3d jest jedynie silnikiem grafiki 3D i nie współpracuje bezposrednio z urządzeniami VR. Jednak producenci gogli zapewniają dedykowane biblioteki dla swoich urządzeń działające w tym silniku, pozwalające na dodanie obsługi wirtualnej rzeczywistosci oraz sterowanie przekazywanym obrazem dla każdego oka osobno. Wykorzystany został pakiet \textit{Oculus Utitites for Unity} udostępniany przez producenta gogli OculusRift (https://developer.oculus.com/downloads/unity/), który w samym edytorze silnika Unity nosi nazwę \textit{OVR plugin}.  \\
